def message_of_today(lst_length, lst_total_length, halfmem_length):
    ...
    if lst_length < (lst_total_length * 0.1):
        print(f"Congrats, you are so close! {percentage:.1f}% left!")
    elif lst_length < (lst_total_length * 0.25):
        print(f"Congrats, less than one-quarter left! {percentage:.1f}% left so far!")
        ...
        print(f"You can do it! {percentage:.1f}% left so far!")
